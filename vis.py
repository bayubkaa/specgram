...
data_end = 1000 # select end of data rows you want to pull from that chunk
data_interval = 100 # select interval you'd like to pull (smaller interval with more loops may run faster)

eqpath = eq1_sig_path # select path to data chunk

img_save_path = 'waves/EQ'
...
for n in range(0,len(starts)):
    ...
    def make_images(i):
        ...
        try:
            ...
            #             hspace = 0, wspace = 0)
            # plt.margins(0,0)
            # plt.savefig(img_save_path+traces[i]+'asli.png',bbox_inches='tight',dpi=50)
            # plt.close()
            
            fig, ax = plt.subplots(figsize=(3,2))
            ax.specgram(data[:,2],Fs=100,NFFT=256,cmap=None,vmin=-10,vmax=25)
            ax.set_xlim([0,60])
            ax.axis('off')
            plt.gca().set_axis_off()
            plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0,
            ...
            #img = cv2.imread(img_save_path+traces[i]+'.png')
            #img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
            #cv2.imwrite(img_save_path+traces[i]+'.png', img)
