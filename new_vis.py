...
earthquakes_1 = earthquakes_1[(earthquakes_1.trace_category == 'earthquake_local') & 
                            ((earthquakes_1.source_distance_km >= 0) & (earthquakes_1.source_distance_km <= 150)) & 
                            ((earthquakes_1.source_magnitude >= 0) & (earthquakes_1.source_magnitude <= 5))]
...
data_end = 10000 # select end of data rows you want to pull from that chunk
...
for n in range(0,len(starts)):
    ...
    mag = magnitude[int(starts[n]):int(ends[n])]
    path = eqpath
    count += 1
    ...
    def make_images(i):
        ...
        try:
            ...
            plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0,
                        hspace = 0, wspace = 0)
            plt.margins(0,0)
            plt.savefig(img_save_path+traces[i]+'_ori.png',bbox_inches='tight',dpi=50)
            plt.close()
